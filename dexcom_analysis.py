...
def main():
    ...
    parser = argparse.ArgumentParser(
                    prog='DexcomDataAnalyzer',
                    description='Analyzes a user\'s Dexcom data export and generates charts')

    parser.add_argument('-s', '--save', action='store_true', help='Save generated charts')
    parser.add_argument('-f', '--filename', default='export.csv', help='Name of the data file')
    ...
    charts = alt.vconcat(glucose, long_acting, fast_acting, carbs).to_html()

    # Display plots
    webview.create_window('Dexcom Data Analysis', html=charts, width=screen_width, height=screen_heigth-50)
    webview.start()

    # Save charts if flag is set
    if args.save:
        glucose.save('out/mean_glucose.png')
        long_acting.save('out/long_acting.png')
        fast_acting.save('out/fast_acting.png')
        carbs.save('out/carbs.png')
